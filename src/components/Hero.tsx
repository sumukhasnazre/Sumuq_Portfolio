...
export const Hero: React.FC<{ theme: "dark" | "light" }> = ({ theme }) => {
  ...
  // Specific transform for the background image to fade slightly faster than text
  const imageOpacity = useTransform(scrollY, [0, 300], [0.4, 0]);
    ...
    <section className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      >
        <img
          ...
          className={`absolute right-[-10%] top-1/2 -translate-y-1/2 h-full w-auto object-contain opacity-40 blur-[1px] lg:blur-none transition-all duration-700 ${theme === "dark" ? "grayscale brightness-[0.6] sepia-[0.3]" : "grayscale-0 brightness-110"}`}
      ...
      <div className="container mx-auto px-6 relative z-10">
        <motion.div style={{ opacity, scale, y }} className="max-w-3xl">
          <motion.div
          ...
          >
            <span className="relative flex h-2 w-2">
              <span
                className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${theme === "dark" ? "bg-blue-400" : "bg-blue-500"}`}
              ></span>
              <span
                className={`relative inline-flex rounded-full h-2 w-2 ${theme === "dark" ? "bg-blue-500" : "bg-blue-600"}`}
              ></span>
            </span>
