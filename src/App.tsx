...
export default function App() {
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-blue-500/30 transition-colors duration-500 ${
      theme === 'dark' ? 'bg-slate-950 text-slate-200 selection:text-white' : 'bg-slate-50 text-slate-800 selection:text-blue-900'
    }`}>
      <AnimatePresence>
      ...
      {!showSplash && (
        ...
        >
                ...
                {resumeData.top_achievements.map((item, i) => (
                  <div key={i} className="flex gap-4 items-start">
                    ...
                    <div>
                      ...
                      <div className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>{item.description}</div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {resumeData.projects.map((project, i) => (
                  <motion.div
                    ...
                    className={`p-8 rounded-2xl border transition-all ${
                      theme === 'dark' ? 'border-white/5 bg-white/[0.02] hover:bg-white/[0.05]' : 'border-black/5 bg-white hover:bg-slate-50 shadow-sm'
                    }`}>
            ...
            <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16">
              <div>
                ...
                <div className="grid gap-4">
                  {resumeData.certifications.map((cert, i) => (
                    <div key={i} className={`p-5 rounded-xl border flex items-center gap-4 ${theme === 'dark' ? 'border-white/5 bg-white/[0.02]' : 'border-black/5 bg-white shadow-sm'}`}>
              ...
              <div>
                ...
                {resumeData.education.map((edu, i) => (
                  <div key={i} className={`p-8 rounded-xl border ${theme === 'dark' ? 'border-blue-500/20 bg-blue-500/5' : 'border-blue-200 bg-blue-50'}`}>
                ...
                <div className="flex flex-wrap gap-3">
                  {Object.entries(resumeData.skills).flatMap(([_, skills]) => skills).map((skill, i) => (
                    <span key={i} className={`px-4 py-2 rounded-lg border text-sm transition-all cursor-default ${theme === 'dark' ? 'bg-white/5 border-white/10 text-slate-300 hover:border-blue-500/30' : 'bg-white border-black/10 text-slate-600 hover:border-blue-300 shadow-sm'}`}>
                      {skill}
                    </span>
                  ))}
                </div>
          ...
          {/* Footer - Contact */}
          <footer className={`py-24 border-t transition-colors ${theme === 'dark' ? 'border-white/5' : 'border-black/5 bg-slate-100'}`}>
            <div className="container mx-auto px-6 text-center">
              ...
              <div className="flex flex-wrap justify-center gap-8 mb-16">
                <a href={`mailto:${resumeData.basics.email}`} className={`flex items-center gap-2 transition-all font-medium ${theme === 'dark' ? 'text-slate-400 hover:text-blue-400' : 'text-slate-500 hover:text-blue-600'}`}>
                  <Mail className="w-5 h-5" /> {resumeData.basics.email}
                </a>
                <a href={`tel:${resumeData.basics.phone}`} className={`flex items-center gap-2 transition-all font-medium ${theme === 'dark' ? 'text-slate-400 hover:text-blue-400' : 'text-slate-500 hover:text-blue-600'}`}>
                  <Smartphone className="w-5 h-5" /> {resumeData.basics.phone}
                </a>
              </div>

              <div className="flex justify-center gap-4">
                <a href={resumeData.basics.links.github} className={`p-4 rounded-2xl border transition-all ${theme === 'dark' ? 'bg-white/5 hover:bg-white/10 text-white border-white/10' : 'bg-white hover:bg-slate-50 text-slate-900 border-black/10 shadow-sm'}`}>
                  <Github className="w-6 h-6" />
                </a>
                <a href={resumeData.basics.links.linkedin} className={`p-4 rounded-2xl border transition-all ${theme === 'dark' ? 'bg-white/5 hover:bg-white/10 text-white border-white/10' : 'bg-white hover:bg-slate-50 text-slate-900 border-black/10 shadow-sm'}`}>
